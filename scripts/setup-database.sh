...
# Use FQDN for cross-namespace access (maas-api pods in redhat-ai-gateway-infra connect to postgres)
DB_CONNECTION_URL="postgresql://${POSTGRES_USER}:${ENCODED_PASSWORD}@postgres.${INFRA_NAMESPACE}.svc.cluster.local:5432/${POSTGRES_DB}?sslmode=disable"
